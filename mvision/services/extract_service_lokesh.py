...
# # # ======= embedding export (centroid + raw bank) =======
# # # When enabled, we will write a text dump per user to:
# # #   {CROPS_ROOT}/embedding_dumps/{user}_{kind}.txt
# # # Format matches the original "first code": label/type/dim/count/centroid + raw_embeddings rows.
# # DUMP_EMBEDDINGS = True
# # EMBEDDING_DUMP_SUBDIR = "embedding_dumps"
# # EMBEDDING_DUMP_FLOAT_FMT = "{:.6f}"
...
# # yolo_model = None
# # reid_extractor: Optional[TorchreidExtractor] = None
# # reid_extractors: List[TorchreidExtractor] = []
# # face_app = None

# # # CSV for logging crops (capture-time)
# # EMB_CSV = str(Path(CROPS_ROOT) / "embeddings.csv")
...
# #     # YOLO
# #     try:
# #         yolo_model = load_yolo_model(imgsz=YOLO_IMGSZ)
...
# #             det_boxes, det_scores = detect_persons_yolo(yolo_model, frame, imgsz=YOLO_IMGSZ)
# #             det_boxes_i = [tuple(map(int, b)) for b in det_boxes]

# #             faces = detect_faces_raw(frame) if (USE_FACE and (frame_idx % FACE_EVERY == 0)) else []

# #             vis = frame.copy()
# #             for det_idx, (x1, y1, x2, y2) in enumerate(det_boxes_i):
# #                 body_crop = frame[max(0, y1):max(0, y2), max(0, x1):max(0, x2)]
# #                 if body_crop.size == 0:
# #                     continue

# #                 ts = _epoch_ms()
# #                 user_id = current_person_id or -1
# #                 user_name = current_person_name or "unknown"
# #                 path = _gallery_body_dir(user_name) / f"{ts}_cam{cam_idx}_f{frame_idx}_p{det_idx}.jpg"
# #                 row = [
# #                     user_id, user_name, ts, cam_idx, frame_idx, det_idx,
# #                     x1, y1, x2, y2,
# #                     float(det_scores[det_idx]) if det_idx < len(det_scores) else "",
# #                     "", "", str(path), "body",
# #                 ]
# #                 try:
# #                     if io_queue is not None:
...
# #                     for f in faces:
# #                         fb = f["bbox"]
# #                         if not link_face_to_person(fb, (x1, y1, x2, y2)):
# #                             continue
...
# #                         ts2 = _epoch_ms()
# #                         path2 = _gallery_face_dir(user_name) / f"{ts2}_cam{cam_idx}_f{frame_idx}_p{det_idx}.jpg"
# #                         row2 = [
# #                             user_id, user_name, ts2, cam_idx, frame_idx, det_idx,
# #                             fx1, fy1, fx2, fy2,
# #                             float(f.get("score", "")),
# #                             "", "", str(path2), "face",
# #                         ]
# #                         try:
# #                             if io_queue is not None:
# #                                 io_queue.put_nowait({"crop_path": path2, "image": face_crop, "csv_row": row2})
# #                         except Exception:
# #                             pass
...
# # def _dump_embedding_bank(user_name: str,
# #                          kind: str,
# #                          bank: List[np.ndarray],
# #                          centroid: Optional[np.ndarray]) -> Optional[str]:
# #     """Write embeddings dump (centroid + raw bank) in the same style as the original script."""
# #     if not DUMP_EMBEDDINGS:
# #         return None
# #     try:
# #         out_dir = Path(CROPS_ROOT) / EMBEDDING_DUMP_SUBDIR
# #         _ensure_dir(out_dir)
# #         out_path = out_dir / f"{user_name}_{kind}.txt"

# #         def _fmt_vec(v: np.ndarray) -> str:
# #             v = np.asarray(v, dtype=np.float32).reshape(-1)
# #             return " ".join(EMBEDDING_DUMP_FLOAT_FMT.format(float(x)) for x in v.tolist())

# #         with open(out_path, "w", encoding="utf-8") as f:
# #             f.write(f"label: {user_name}\n")
# #             f.write(f"type: {kind}\n")
# #             f.write(f"dim: {EXPECTED_EMBED_DIM}\n")
# #             f.write(f"count: {len(bank)}\n")
# #             f.write("centroid: ")
# #             f.write(_fmt_vec(centroid) if centroid is not None else "")
# #             f.write("\n")
# #             f.write("raw_embeddings:\n")
# #             for i, vec in enumerate(bank):
# #                 f.write(f"{i}\t{_fmt_vec(vec)}\n")
# #         return str(out_path)
# #     except Exception:
# #         logger.exception("Failed to write embeddings dump (user=%s kind=%s)", user_name, kind)
# #         return None
...
# # # ---------- BODY embeddings from gallery (ENSEMBLE + TTA) ----------
# # def _compute_body_bank_and_centroid_from_gallery(user_name: str,
# #                                                  on_step: Optional[Callable[[int], None]] = None
...
# #     return vectors_per_image, _mean_embed(vectors_per_image)


# # def _compute_body_embedding_from_gallery(user_name: str,
# #                                          on_step: Optional[Callable[[int], None]] = None
# #                                          ) -> Optional[np.ndarray]:
# #     """Backward-compatible: return only centroid."""
# #     _bank, centroid = _compute_body_bank_and_centroid_from_gallery(user_name, on_step=on_step)
# #     return centroid


# # # ---------- FACE embeddings from gallery (TTA) ----------
# # def _compute_face_bank_and_centroid_from_gallery(user_name: str,
# #                                                  on_step: Optional[Callable[[int], None]] = None
# #                                                  ) -> Tuple[List[np.ndarray], Optional[np.ndarray]]:
# #     if face_app is None:
# #         logger.warning("InsightFace not available; skipping face embedding.")
...
# #             if best is None:
# #                 return None
# #             det = float(getattr(best, "det_score", 0.0))
# #             if det < FACE_MIN_SCORE:
# #                 return None
...
# #         e1 = None
# #         if FACE_TTA_FLIP:
# #             img_flip = cv2.flip(img_proc, 1)
# #             e1 = _best_face(img_flip)
...
# #         logger.warning("No valid FACE embeddings for '%s'", user_name)
# #         return [], None
# #     return vectors, _mean_embed(vectors)
...
# #                                          ) -> Optional[np.ndarray]:
# #     """Backward-compatible: return only centroid."""
# #     _bank, centroid = _compute_face_bank_and_centroid_from_gallery(user_name, on_step=on_step)
# #     return centroid
...
# #         user = session.query(User).filter(User.id == user_id).first()
# #         if not user:
# #             logger.error("User with id %d not found. Skipping DB update.", user_id)
# #             return {"status": "error", "message": "user not found"}
...
# #             body_bank, body_emb = _compute_body_bank_and_centroid_from_gallery(user_name, on_step=on_body)
# #             body_dump_path = _dump_embedding_bank(user_name, "body", body_bank, body_emb)
# #         else:
# #             _progress_set(user_id, message="Skipping body (no images or model unavailable)")

# #         face_emb = None
# #         face_bank: List[np.ndarray] = []
# #         face_dump_path = None
# #         if total_face > 0 and face_app is not None:
# #             _progress_set(user_id, stage="embedding_face", message=f"Processing face ({total_face})")
...
# #             face_bank, face_emb = _compute_face_bank_and_centroid_from_gallery(user_name, on_step=on_face)
# #             face_dump_path = _dump_embedding_bank(user_name, "face", face_bank, face_emb)
# #         else:
# #             _progress_set(user_id, message="Skipping face (no images or model unavailable)")
...
# #         try:
# #             res.update({
# #                 "body_count": len(body_bank) if "body_bank" in locals() else 0,
# #                 "face_count": len(face_bank) if "face_bank" in locals() else 0,
# #                 "body_dump_path": body_dump_path,
# #                 "face_dump_path": face_dump_path,
# #             })
# #         except Exception:
# #             pass
...
# #     if total_body > 0 and reid_extractors:
# #         body_bank, body_emb = _compute_body_bank_and_centroid_from_gallery(user_name, on_step=lambda n: None)
# #         body_dump_path = _dump_embedding_bank(user_name, "body", body_bank, body_emb)

# #     if total_face > 0 and face_app is not None:
# #         face_bank, face_emb = _compute_face_bank_and_centroid_from_gallery(user_name, on_step=lambda n: None)
# #         face_dump_path = _dump_embedding_bank(user_name, "face", face_bank, face_emb)

# #     res = _update_db_embeddings(user_id, body_emb, face_emb)
# #     try:
# #         res.update({
# #             "body_count": len(body_bank),
# #             "face_count": len(face_bank),
# #             "body_dump_path": body_dump_path,
# #             "face_dump_path": face_dump_path,
# #         })
# #     except Exception:
...
# Raw bank storage sizing (avoid huge rows)
RAW_STORE_LIMIT = int(os.getenv("RAW_STORE_LIMIT", "256"))  # max vectors to store per modality
...
def _pack_raw_bank(vectors: Optional[List[np.ndarray]]) -> Optional[bytes]:
    """
    Stores float32[N,512] as gzip(np.save()) into BYTEA.

    WHY: keep raw banks compact and DB-safe; avoids huge JSON rows.
    """
    if not vectors:
        return None
...
def _update_db_embeddings(
    ...
    try:
        ...
        # Centroids
        if body_emb is not None:
            user.body_embedding = body_emb.tolist()
            changed = True
        if face_emb is not None:
            user.face_embedding = face_emb.tolist()
            changed = True
        ...
        if body_bank is not None:
            user.body_embeddings_raw = _pack_raw_bank(body_bank)
            changed = True
        if face_bank is not None:
            user.face_embeddings_raw = _pack_raw_bank(face_bank)
            changed = True
        ...
        if changed:
            ...
            session.commit()
            logger.info(
                "DB updated: user=%s body_cent=%s face_cent=%s body_raw=%s face_raw=%s",
                user_id,
...
def _extract_worker(user_id: int, user_name: str):
    try:
        ...
        _progress_set(user_id, total_body=total_body, total_face=total_face, done_body=0, done_face=0, percent=0)
        ...
        if total_body > 0 and reid_extractors:
            ...
            body_bank, body_cent = _compute_body_bank_and_centroid_from_gallery(user_name, on_step=on_body)
        else:
            _progress_set(user_id, message="Skipping body (no images or model unavailable)")
        ...
        if total_face > 0 and face_app is not None:
            ...
            face_bank, face_cent = _compute_face_bank_and_centroid_from_gallery(user_name, on_step=on_face)
        else:
            _progress_set(user_id, message="Skipping face (no images or model unavailable)")

        res = _update_db_embeddings(user_id, body_cent, face_cent, body_bank=body_bank, face_bank=face_bank)
        status = str(res.get("status", "unknown"))
        _progress_set(user_id, stage="done", message=("Embeddings saved" if status == "ok" else status), percent=100)
    except Exception as e:
        logger.exception("extract worker failed")
        _progress_set(user_id, stage="error", message=f"error: {e}")
...
def extract_embeddings_sync(user_id: int) -> Dict[str, object]:
    ...
    body_bank, body_cent = (None, None)
    face_bank, face_cent = (None, None)

    if total_body > 0:
        body_bank, body_cent = _compute_body_bank_and_centroid_from_gallery(user_name, on_step=lambda n: None)
    if total_face > 0:
        face_bank, face_cent = _compute_face_bank_and_centroid_from_gallery(user_name, on_step=lambda n: None)
...
def start_extraction(user_id: int, show_viewer: bool = True) -> dict:
    ...
    for cam_idx, rtsp_url in enumerate(RTSP_STREAMS):
        cap_queues[cam_idx] = Queue(maxsize=CAP_QUEUE_MAX)
        t_cap = threading.Thread(target=capture_thread_fn, name=f"cap_{cam_idx}", args=(cam_idx, rtsp_url), daemon=True)
        t_ext = threading.Thread(
            target=embedding_loop_for_cam, name=f"ext_{cam_idx}", args=(cam_idx, rtsp_url), daemon=True
        )
        capture_threads[cam_idx] = t_cap
        extract_threads[cam_idx] = t_ext
        t_cap.start()
    ...
    _progress_set(int(user_id), stage="running", message="processing")
    logger.info("Extraction started for user_id=%s name=%s on %d stream(s)", user_id, user_name, len(RTSP_STREAMS))

    return {"status": "ok", "message": "started", "num_cams": len(RTSP_STREAMS), "id": int(user_id), "name": user_name}
...
def remove_embeddings(id: int):
    """Remove stored embeddings (centroids + raw banks)."""
    session = get_session()
    try:
        user = session.query(User).filter(User.id == id).first()
        if not user:
        ...
        user.body_embedding = None
        user.face_embedding = None
        user.body_embeddings_raw = None
        user.face_embeddings_raw = None
        user.last_embedding_update_ts = datetime.now(timezone.utc)
...
# FACE_MODEL = "buffalo_l"
# FACE_DET_SIZE = (
#     int(os.getenv("FACE_DET_W", "320" if FAST_MODE else "640")),
#     int(os.getenv("FACE_DET_H", "320" if FAST_MODE else "640")),
# )  # (w, h)
# FACE_PROVIDER = "auto"  # "auto" | "cuda" | "cpu"
# FACE_MIN_SCORE = 0.5
# FACE_MIN_SIZE = 32  # px min(side) of detected face
...
# # Queues
# CAP_QUEUE_MAX = 2
# IO_WORKERS = int(os.getenv("IO_WORKERS", "4" if FAST_MODE else "2"))
# CSV_FLUSH_EVERY = int(os.getenv("CSV_FLUSH_EVERY", "64"))
# CSV_FLUSH_SECS = float(os.getenv("CSV_FLUSH_SECS", "0.5"))
# JPEG_QUALITY = int(os.getenv("JPEG_QUALITY", "85"))
# IO_DRAIN_TIMEOUT = float(os.getenv("IO_DRAIN_TIMEOUT", "10"))

# IO_QUEUE_MAX = int(os.getenv("IO_QUEUE_MAX", "4096"))

# # Viewer FPS throttle
# VIEWER_SLEEP_MS = 5

# # ======= embeddings perf/quality knobs =======
# REID_MODELS = [
#     m.strip()
#     for m in os.getenv(
#         "REID_MODELS",
#         ("osnet_x0_25" if FAST_MODE else "osnet_x1_0,osnet_x0_25"),
#     ).split(",")
#     if m.strip()
# ]
# BODY_TTA_FLIP = os.getenv("BODY_TTA_FLIP", ("0" if FAST_MODE else "1")) == "1"
# FACE_TTA_FLIP = os.getenv("FACE_TTA_FLIP", ("0" if FAST_MODE else "1")) == "1"
# BODY_BATCH = int(os.getenv("BODY_BATCH", "128" if FAST_MODE else "64"))
# FACE_MAX_SIDE = int(os.getenv("FACE_MAX_SIDE", "320" if FAST_MODE else "640"))
# MAX_GALLERY_IMAGES = int(os.getenv("MAX_GALLERY_IMAGES", "0"))  # 0=all; else sample up to N
# STORE_ALL_RAW = os.getenv("STORE_ALL_RAW", "0") == "1"
# COMPRESS_LEVEL = int(os.getenv("COMPRESS_LEVEL", "1" if FAST_MODE else "6"))
...
#     Percent is always computed over the full job:
#     (done_body + done_face) / (total_body + total_face).
#     This keeps the progress bar monotonic and accurate across stages.
#     """
#     with progress_lock:
#         st = progress_state.get(user_id)
#         if not st:
...
#     buf = io.BytesIO()
#     np.save(buf, arr, allow_pickle=False)
#     return gzip.compress(buf.getvalue(), compresslevel=max(1, min(9, int(COMPRESS_LEVEL))))
...
# def _gpu_enabled(device: object = DEVICE) -> bool:
#     """Return True if CUDA should be used."""
#     dev = str(device).strip().lower()
#     return torch.cuda.is_available() and (FORCE_CUDA or ("cuda" in dev) or (dev in ("auto", "gpu")))
...
#     """Start IO workers (image writer + CSV writer) once."""
#     global io_queue, csv_queue, io_threads, csv_thread

#     ensure_csv_header()

#     if io_queue is None:
#         io_queue = Queue(maxsize=int(IO_QUEUE_MAX))
#     if csv_queue is None:
#         csv_queue = Queue(maxsize=int(IO_QUEUE_MAX))

#     io_threads = [t for t in io_threads if t.is_alive()]

#     if csv_thread is None or not csv_thread.is_alive():
#         csv_thread = threading.Thread(target=csv_worker, name="csv_worker", daemon=True)
#         csv_thread.start()

#     while len(io_threads) < max(1, int(IO_WORKERS)):
#         t = threading.Thread(target=io_worker, name=f"io_worker_{len(io_threads)}", daemon=True)
#         t.start()
#         io_threads.append(t)


# def io_worker() -> None:
#     """Write crops to disk, then enqueue CSV rows.

#     Drains pending jobs even after stop_event is set, so auto-extract sees all crops.
#     """
#     assert io_queue is not None
#     assert csv_queue is not None

#     params = [int(cv2.IMWRITE_JPEG_QUALITY), int(max(1, min(100, JPEG_QUALITY)))]

#     while True:
#         if stop_event.is_set() and io_queue.empty():
#             break
#         try:
#             job = io_queue.get(timeout=0.1)  # type: ignore
#         except Empty:
...
#             _ensure_dir(crop_path.parent)
#             img = job["image"]
#             ok = cv2.imwrite(str(crop_path), img, params) if str(crop_path).lower().endswith((".jpg", ".jpeg")) else cv2.imwrite(str(crop_path), img)
#             if not ok:
#                 raise RuntimeError(f"cv2.imwrite failed: {crop_path}")
#             csv_queue.put(job["csv_row"])
#         except Exception:
#             logger.exception("IO worker error")
#         finally:
#             try:
...
# def csv_worker() -> None:
#     """Batch CSV writes for high throughput."""
#     assert csv_queue is not None
...
#             w = csv.writer(f)
#             while True:
#                 if stop_event.is_set() and csv_queue.empty():
#                     if buf:
#                         w.writerows(buf)
#                         f.flush()
#                         buf.clear()
...
#                 now = time.time()
#                 if buf and (len(buf) >= int(CSV_FLUSH_EVERY) or (now - last_flush) >= float(CSV_FLUSH_SECS)):
#                     w.writerows(buf)
#                     f.flush()
#                     buf.clear()
#                     last_flush = now
#     except Exception:
#         logger.exception("CSV worker error")


# def drain_io(timeout: float = IO_DRAIN_TIMEOUT) -> None:
#     """Best-effort drain of pending IO jobs before running embedding extraction."""
#     start = time.time()
#     while True:
#         pending = 0
#         try:
#             if io_queue is not None:
#                 pending += int(getattr(io_queue, "unfinished_tasks", 0))
#             if csv_queue is not None:
#                 pending += int(getattr(csv_queue, "unfinished_tasks", 0))
#         except Exception:
#             pending = 0
...
#             logger.warning("IO drain timeout reached; continuing (pending=%s).", pending)
#             return
#         time.sleep(0.05)
...
#                             imgsz=YOLO_IMGSZ,
#                             verbose=False,
#                             device=DEVICE if _gpu_enabled(DEVICE) else "cpu",
#                         )
#                     boxes = res[0].boxes if (res and len(res)) else None
#                     if boxes is not None:
#                         xyxy = boxes.xyxy.detach().cpu().numpy().astype(np.float32)
...
#                     fx1c, fy1c, fx2c, fy2c = max(0, fx1), max(0, fy1), min(W, fx2), min(H, fy2)
#                     if fx2c <= fx1c or fy2c <= fy1c:
#                         continue
#                     face_crop = frame[fy1c:fy2c, fx1c:fx2c]
#                     if face_crop.size <= 0:
#                         continue
#                     fname = f"person_{_epoch_ms()}.jpg"
#                     path = face_dir / fname
...
#                     try:
#                         if io_queue is not None:
#                             io_queue.put_nowait({"crop_path": path, "image": face_crop, "csv_row": row})
#                     except Exception:
#                         pass
...
#     body = len(_list_images(_gallery_body_dir(user_name)))
#     face = len(_list_images(_gallery_face_dir(user_name)))
#     if int(MAX_GALLERY_IMAGES) > 0:
#         body = min(body, int(MAX_GALLERY_IMAGES))
#         face = min(face, int(MAX_GALLERY_IMAGES))
#     return body, face
...
#     on_step: Optional[Callable[[int], None]] = None,
# ) -> Tuple[Optional[List[np.ndarray]], Optional[np.ndarray]]:
#     """Compute body embeddings from gallery crops.

#     - Uses GPU when available.
#     - Streams images in batches so memory stays bounded.
#     - Updates progress *after* each file is fully processed (skip or embedded).
#     """
#     if not reid_extractors:
#         logger.warning("TorchReID not available; skipping body embedding.")
#         return None, None
...
#     if int(MAX_GALLERY_IMAGES) > 0 and len(paths) > int(MAX_GALLERY_IMAGES):
#         idxs = np.linspace(0, len(paths) - 1, int(MAX_GALLERY_IMAGES), dtype=int).tolist()
#         paths = [paths[i] for i in idxs]

#     gpu = _gpu_enabled(DEVICE)
#     infer_bs = int(BODY_BATCH) if gpu else 32
#     tta_mult = 2 if BODY_TTA_FLIP else 1
#     file_bs = max(4, infer_bs // tta_mult)

#     bank_for_store: List[np.ndarray] = []
#     sum_vec = np.zeros((EXPECTED_EMBED_DIM,), dtype=np.float32)
#     n_vec = 0
...
#         if gpu and hasattr(torch, "autocast")
#         else None
#     )

#     def _embed_with_extractor(ext: TorchreidExtractor, imgs: List[np.ndarray]) -> List[np.ndarray]:
#         out: List[np.ndarray] = []
#         for start in range(0, len(imgs), infer_bs):
#             chunk = imgs[start:start + infer_bs]
#             try:
#                 with reid_lock, torch.inference_mode():
...
#                             feats = ext(chunk)
#                 for f in feats:
#                     arr = f.detach().cpu().numpy() if hasattr(f, "detach") else np.asarray(f)
#                     f512 = _as_512f(arr)
#                     out.append(f512 if f512 is not None else np.zeros((EXPECTED_EMBED_DIM,), np.float32))
#             except Exception:
#                 logger.exception("TorchReID batch failed")
#                 out.extend([np.zeros((EXPECTED_EMBED_DIM,), np.float32) for _ in chunk])
#         return out

#     for start in range(0, len(paths), file_bs):
#         chunk_paths = paths[start:start + file_bs]
...
#         per_model_fused: List[List[np.ndarray]] = []
...
#             feats_norm = _embed_with_extractor(ext, batch_imgs)

#             img_accum: List[List[np.ndarray]] = [[] for _ in prepared]
#             for (img_i, _tta_i), feat in zip(idx_map, feats_norm):
#                 img_accum[img_i].append(feat)
...
#         for i in range(len(prepared)):
#             parts = [per_model_fused[m][i] for m in range(len(per_model_fused))]
#             fused_img = l2_normalize(np.mean(np.stack(parts, axis=0), axis=0))
#             sum_vec += fused_img.astype(np.float32)
#             n_vec += 1
#             if STORE_ALL_RAW or (len(bank_for_store) < int(RAW_STORE_LIMIT)):
#                 bank_for_store.append(fused_img.astype(np.float32))
#             if on_step:
#                 on_step(1)

#     centroid = l2_normalize(sum_vec / float(max(1, n_vec))) if n_vec > 0 else None
#     return (bank_for_store if bank_for_store else None), centroid
...
#     on_step: Optional[Callable[[int], None]] = None,
# ) -> Tuple[Optional[List[np.ndarray]], Optional[np.ndarray]]:
#     """Compute face embeddings from face-crop gallery images."""
#     if face_app is None:
#         logger.warning("InsightFace not available; skipping face embedding.")
#         return None, None
...
#             try:
#                 faces = face_app.get(np.ascontiguousarray(img))
#             except Exception:
#                 faces = []
...
#             if best is None:
#                 continue
#             if float(getattr(best, "det_score", 0.0)) < float(FACE_MIN_SCORE):
#                 continue

#             e0 = _as_512f(extract_face_embedding(best))
#             e1 = None
#             if FACE_TTA_FLIP:
#                 try:
#                     faces2 = face_app.get(np.ascontiguousarray(cv2.flip(img, 1)))
#                 except Exception:
#                     faces2 = []
#                 best2 = None
#                 best_rank2 = -1.0
#                 for f in safe_iter_faces(faces2):
#                     bbox = getattr(f, "bbox", None)
#                     if bbox is None:
#                         continue
#                     b = np.asarray(bbox).reshape(-1)
...
#                     rank = score * (area ** 0.5)
#                     if rank > best_rank2:
#                         best2, best_rank2 = f, rank
#                 if best2 is not None and float(getattr(best2, "det_score", 0.0)) >= float(FACE_MIN_SCORE):
#                     e1 = _as_512f(extract_face_embedding(best2))
...
#             sum_vec += fused.astype(np.float32)
#             n_vec += 1
#             if STORE_ALL_RAW or (len(bank_for_store) < int(RAW_STORE_LIMIT)):
#                 bank_for_store.append(fused.astype(np.float32))
#         finally:
#             if on_step:
#                 on_step(1)
...
#     face_bank: Optional[List[np.ndarray]] = None,
# ) -> Dict[str, object]:
#     """Persist embeddings to DB in a single UPDATE for speed."""
#     session = get_session()
#     try:
#         payload: Dict[str, object] = {}
#         if body_emb is not None:
#             payload["body_embedding"] = body_emb.tolist()
#         if face_emb is not None:
#             payload["face_embedding"] = face_emb.tolist()
#         if body_bank is not None:
#             payload["body_embeddings_raw"] = _pack_raw_bank(body_bank)
#         if face_bank is not None:
#             payload["face_embeddings_raw"] = _pack_raw_bank(face_bank)

#         if not payload:
#             logger.info("No embeddings to update for user=%s", user_id)
#             return {"status": "no_embeddings"}

#         payload["last_embedding_update_ts"] = datetime.now(timezone.utc)

#         updated = session.query(User).filter(User.id == user_id).update(payload, synchronize_session=False)
#         if not updated:
#             logger.warning("User ID %s not found. Skipping DB update.", user_id)
#             session.rollback()
#             return {"status": "error", "message": "user not found"}
...
#     Important: drains pending crop/CSV IO before triggering auto-extract so embeddings
#     are computed from the full gallery (no missing files).
#     """
#     global is_running, current_person_id, current_person_name
#     try:
#         uid_snapshot = current_person_id
...
#         # Finish writing any queued crops/rows.
#         drain_io(timeout=float(IO_DRAIN_TIMEOUT))
...
# def get_embedding_storage_info(user_id: int) -> Dict[str, object]:
#     """Debug helper: verify what is stored in DB for a user."""
#     session = get_session()
#     try:
#         user = session.query(User).filter(User.id == int(user_id)).first()
#         if not user:
...
#         def _raw_count(raw: Optional[bytes]) -> int:
#             if not raw:
#                 return 0
#             try:
#                 arr = np.load(io.BytesIO(gzip.decompress(raw)), allow_pickle=False)
#                 return int(getattr(arr, "shape", [0])[0])
#             except Exception:
#                 return 0
...
#             "status": "ok",
#             "id": int(user_id),
#             "has_body_centroid": user.body_embedding is not None,
#             "has_face_centroid": user.face_embedding is not None,
#             "body_raw_count": _raw_count(getattr(user, "body_embeddings_raw", None)),
#             "face_raw_count": _raw_count(getattr(user, "face_embeddings_raw", None)),
#             "last_embedding_update_ts": str(getattr(user, "last_embedding_update_ts", None)),
#         }
#     except Exception:
#         session.rollback()
#         logger.exception("get_embedding_storage_info: DB error")
#         return {"status": "error", "message": "DB error"}
#     finally:
#         session.close()
